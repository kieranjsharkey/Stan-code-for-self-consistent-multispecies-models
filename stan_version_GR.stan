functions {
  ...
  vector[2] dydt;
  dydt[1] = alpha_A * (y[1]) * log(kappa_A*kappa_B/(kappa_B*y[1]+beta_AB*kappa_A*y[2]));
  dydt[2] = alpha_B * (y[2]) * (1 - (y[2] / kappa_B + beta_BA * (y[1] / kappa_A)) ^ nu_B);
...
parameters {
  ...
  real<lower=270,upper=380> epsilon_A_mean;
  real<lower=0.088,upper = 0.11> epsilon_B_mean;
  ...
  vector<lower=270,upper=380>[n_wells] epsilon_A;
  vector<lower=0.088,upper = 0.11>[n_wells] epsilon_B;
}
...
model {
  ...
  // Define priors
  sigma_B ~ exponential(1000);
  epsilon_A_mean ~ normal(331.16,12.94);
  epsilon_B_mean ~ normal(0.09555,0.001723);
  epsilon_A_sd ~ exponential(1);
  epsilon_B_sd ~ exponential(1);

  kappa_A_mean ~ normal(0.985,0.1);
  kappa_B_mean ~ normal(1.083,0.1);
  kappa_A_sd ~ exponential(100);
  kappa_B_sd ~ exponential(100);

  B0_A_mean ~ normal(0.001,0.001); // Informed from SS
  B0_B_mean ~ normal(0.001,0.001);
  B0_A_sd ~ exponential(50);
  B0_B_sd ~ exponential(50);

  alpha_A_mean ~ normal(2,1);
  alpha_B_mean ~ normal(2,1);
  ...
  // biomass_scale ~ normal(0.1,1);
  // B0_A_mean ~ uniform(0.00001,0.1);
  // B0_B_mean ~ uniform(0.00001,0.1);
  // kappa_A_mean ~ uniform(0.7,2);
  // kappa_B_mean ~ uniform(0.7,2);
  // alpha_A ~ uniform(0.1,6);
  // alpha_B ~ uniform(0.1,6);
  // nu_A ~ uniform(0.1,5);
  // nu_B ~ uniform(0.1,5);
  // beta_AB ~ uniform(0.1,2);
  // beta_BA ~ uniform(0.1,2);
  ...
  for (well in 1:n_wells){
    ...
    // array[2*T] vector[2] mu = ode_rk45(ode_function, y0[well,], 0, ts,
                                     alpha_A[well],
                                     alpha_B[well],
                                     // nu_A,
                                     nu_B,
