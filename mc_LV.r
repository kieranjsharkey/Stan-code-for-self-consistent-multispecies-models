...
file <- file.path(paste0(current_wd,"/stan_version_LV.stan"))
...
write.csv(draws, file = "draws_full_model_LV.csv")


mcmc_hist(fit$draws("kappa_B_mean"))
pdf("Traces_LV.pdf",onefile = TRUE)
mcmc_trace(fit$draws(), pars = c("alpha_B_mean","alpha_A_mean","kappa_B_mean","kappa_A_mean"))
mcmc_trace(fit$draws(), pars = c("beta_AB","beta_BA","B0_B_mean","B0_A_mean"))
