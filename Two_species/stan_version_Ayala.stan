functions {
  ...
  dydt[1] = alpha_tau * (y[1]) * (1 - (y[1] / kappa_tau)^nu_tau - beta_tau_omega * y[2] / kappa_omega);
  dydt[2] = alpha_omega * (y[2]) * (1 - (y[2] / kappa_omega)^nu_omega - beta_omega_tau * y[1] / kappa_tau);
...
parameters {
  real<lower=0> sigma;
  real<lower=230,upper=380> delta_mean;
  ...
  vector<lower=230,upper=380>[n_wells] delta;
