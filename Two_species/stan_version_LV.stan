functions {
  ...
  dydt[1] = alpha_tau * (y[1]) * (1 - (y[1] / kappa_tau) - beta_tau_omega * (y[2] / kappa_omega));
  dydt[2] = alpha_omega * (y[2]) * (1 - (y[2] / kappa_omega) - beta_omega_tau * (y[1] / kappa_tau));
...
parameters {
  real<lower=0> sigma;
  real<lower=0,upper=380> delta_mean;
  real<lower=0,upper = 0.11> epsilon_mean;
  ...
  vector<lower=0,upper=380>[n_wells] delta;
  vector<lower=0,upper = 0.11>[n_wells] epsilon;
}
...
model {
  ...
  delta_sd ~ exponential(100);
  epsilon_sd ~ exponential(100);

  kappa_tau_mean ~ normal(0.985,0.1);
  kappa_omega_mean ~ normal(1.083,0.1);
  kappa_tau_sd ~ exponential(1000);
  kappa_omega_sd ~ exponential(1000);

  c_tau_mean ~ normal(0.01,0.01); // Informed from SS
  c_omega_mean ~ normal(0.01,0.01);
  c_tau_sd ~ exponential(50);
  c_omega_sd ~ exponential(50);

  alpha_tau_mean ~ normal(0.8,1);
  alpha_omega_mean ~ normal(0.8,1);
